...
3. **Palette confirmation.** Every label is champagne-gold typography on black, and so is the brand lockup — the current mark is gold line art on a transparent ground with no disc and no ring (the navy-disc/magenta-ring icon described in earlier revisions of this file is retired; see §1). The brief's palette (gold primary, near-black ground) is therefore *literally* on the packaging. Gold measured from the wordmark PNG: p10 `#7D510A`, p50 `#E0D392`, p90 `#FEF990` (a pale champagne at the top end — use `#F5D76E` for UI gold and `#FFEFA6` for highlights, both inside the logo's own range).
...
### Logo (wordmark) — `https://res.cloudinary.com/v8vrixwq/image/upload/v1789379844/new_logo.png`
- 2073 × 758 px canvas, RGBA, 713 KB. **The art does not fill it**: the ink's bounding box is 1923 × 502 at x 72–1995, y 98–600, i.e. a wide transparent bleed, asymmetric top to bottom. Delivered aspect is therefore the TRIMMED **3.83 : 1**, not the canvas's 2.73 : 1 — see `brand.logoAspect` and the `trim` option on `cld()`. Every consumer emits `e_trim`; nothing renders the raw canvas.
- Alpha: **79.5 % fully transparent, 20.5 % partial, ~0 % fully opaque** → a transparent ground with soft, fully anti-aliased edges (the export carries no hard-opaque core, so never key it against `alpha == 255`). **On `#17120F` the wordmark sits directly on the chrome with no plate.**
- Colour: gold gradient over the ink (p10 `#7D510A` → p50 `#E0D392` → p90 `#FEF990`). Median gold measures **12.3 : 1 on `#17120F`** and **11.4 : 1 on the `#201A15` surface** — no plate or glow is needed for legibility on either. On the CREAM page ground `#F8F3EA` it is **1.4 : 1**, a watermark: any light surface that carries the lockup must put it back on a dark plate first (this is what `.sf-lockup-plate` + `.sf-on-dark` do in the auth modal, and why the splash screen is dark).
- Composition: botanical mark (tulip-like bud with two leaves) left, "LAMIKAA" serif caps with a swash from the K into the final A, "NATURALS" tracked caps beneath. Minimum render width for the tagline to stay legible: **≈ 150 px**; header uses 168 px (desktop) and 140 px (mobile), admin login 230 px; below ~120 px switch to the icon.

### Icon (mark) — `https://res.cloudinary.com/v8vrixwq/image/upload/v1789379845/new_logo_image.png`
- 1254 × 1254 px canvas, RGBA, 929 KB. Ink bbox 1024 × 1126 at x 124–1148, y 96–1222 — **not square, and not centred**: it is taller than it is wide and sits low in the frame. Anything painting it into a square slot must trim and re-pad (`trim: true` + `ar: "1:1"`), or the mark renders small and low.
- **72.4 % fully transparent, 27.5 % partial, ~0 % fully opaque.** No disc, no ring, no fill of any kind — the mark is the gold botanical line-drawing alone (p10 `#773800` → p50 `#FCCC23` → p90 `#FCFC8D`), which is a change from the previous icon and is why nothing pads it on `b_auto`: a sampled ground would paint an opaque plate behind line art drawn for none.
- Median gold measures **12.2 : 1 on `#17120F`**, so the mark reads on the chrome unaided wherever it appears in the UI (compact header mark under 340 px, admin collapsed sidebar) at ≤ 40 px. Do not recolour it.
- Favicon generation: `npm run icons` (`scripts/generate-icons.js`) writes all seven files from this URL and verifies the bytes it got back. The transparent set (16/32, 48 ICO, 180 Apple, 192/512 PWA `purpose: any`) is `e_trim` then `c_lpad,w_1200,h_1200,b_transparent` — 1126 of ink in a 1200 frame, a ~94 % fill that matches what the previous icon's own margin gave. The `maskable` variant is the same trim on `c_lpad,w_1480,h_1480,b_rgb:17120F` — a ~76 % fill, inside Android's 80 % safe zone, on the chrome ground that `theme_color` and `background_color` also carry.
...
| Gold on pack is pale champagne (`#7D510A`–`#FEF990`) | UI gold `#F5D76E` / highlight `#FFEFA6` / dark gold `#B88924` as briefed; eyebrows and prices use `#F5D76E`; gradient text may use the lighter logo range. |
| The pink/violet accents no longer come from the mark | The icon that carried a magenta→violet ring is retired (§1): today's mark is gold line art alone. Neon pink `#FF4FD8` / violet `#8B5CF6` and the signature gold→pink→violet gradient STAY — they are the brief's own accent palette and are load-bearing across the token layer — but they are now a design-system choice, not something the artwork evidences. |
