...
- `36 · 2026-09-08 · The og:url and twitter:url tags were REMOVED from public/index.html rather than resolved · The prompt offers exactly two outcomes and PLACEHOLDERS.md settles which one applies: brand.seo.siteUrl is still `{{LAMIKAA_DOMAIN}}`, the owner has not named the public domain, and the guardrail forbids resolving a placeholder with a guess to make a grep pass. A literal token in a crawled tag is worse than no tag — a crawler reads `https://{{LAMIKAA_DOMAIN}}/` as the canonical URL of the site. Nothing is lost by removing them: since Prompt 08 `useSeo` claims `og:url` and `<link rel=canonical>` on EVERY route from `seoOrigin()`, which returns `brand.seo.siteUrl` the moment it stops being a placeholder and `window.location.origin` until then, so the runtime tag was already more correct than the static one it borrowed. `twitter:url` is not read by any card renderer (the card follows og:url/canonical), so it had no replacement to need. Re-adding a static pair once the domain is known is optional and belongs with Prompt 38's sitemap work; PLACEHOLDERS.md's `{{LAMIKAA_DOMAIN}}` row now reads "`brand.js → seo.siteUrl` — and nowhere else since Prompt 36".
- `36 · 2026-09-08 · robots.txt's `Sitemap:` line was dropped now rather than left for Prompt 38 · Task 2 makes it conditional ("keep it only if Prompt 38 will resolve the domain"), and Prompt 38's own Task 5 writes `public/sitemap.xml` only when `brand.seo.siteUrl` is real and carries the `Sitemap:` line "only when the file exists". With the domain unresolved, 38 cannot resolve the host either, so keeping the line would have shipped `Sitemap: https://{{LAMIKAA_DOMAIN}}/sitemap.xml` to crawlers through two more prompts. robots.txt is served verbatim and has no comment-stripping build step, so the replacement comment names the token as bare `LAMIKAA_DOMAIN` rather than in braces — the file now contains no brace-pair at all. The same rewording was applied to the explanatory comment left in `public/index.html`.
- `36 · 2026-09-08 · package-lock.json's root name was the ONLY lockfile change, and `npm install --package-lock-only` produced exactly that · The prompt asks for the name to match and warns to revert if more moves. `git diff --stat package-lock.json` → `4 ++--` = the two `"name": "ecommerce-boilerplate"` lines (root and `packages[""]`) becoming `lamikaa-naturals-storefront`. No dependency, version or integrity line changed, even though this container runs npm 10.9.7 against the 10.9.2 that wrote the lock. Nothing reverted, nothing to record beyond this.
- `36 · 2026-09-08 · Two admin routes DO render a `{{TOKEN}}`, and that is the designed behaviour, not a leak · The acceptance criterion reads "no `{{` renders on any storefront or admin route"; the route walk found `/admin/announcements` and `/admin/faqs` printing `{{FREE_SHIPPING_THRESHOLD}}` and `{{RETURN_WINDOW_DAYS}}`. Both are the STORED RECORD shown in the editor that exists to change it — the announcement row sits beside its own "Placeholder — FREE_SHIPPING_THRESHOLD" chip and a "Hidden" status, exactly as Prompt 34 built it. An owner cannot replace a placeholder they cannot see, so hiding the token there would remove the feature to make a check pass, which the guardrails forbid outright. The check that actually matters was run and passes: the same two records render as "Farmer-owned. Assam-grown." (the unresolved row dropped by the bar) and "You can request a return from My Orders within 7 days of delivery" on the storefront, and `document.body.innerText.includes("{{")` is false on all 25 storefront routes at both widths. `/admin/content`'s token cheat-sheet (`CONTENT_TOKENS` in `MarkdownField.js`) is the same case and does not even render by default — it lives behind the collapsed help panel.
- `36 · 2026-09-08 · Three admin PROSE strings that quoted the token syntax were reworded; the cheat-sheet was not touched · `AdminAnnouncements.js` explained itself with a literal `<code>{{TOKEN}}</code>`, a `{{UPPER_SNAKE}}` tooltip and a `{{UPPER_SNAKE}}` error alert. The first renders unconditionally whenever any announcement carries a placeholder — the seeded state — so it put a brace-pair in the DOM and in the bundle for no reason a sentence could not carry ("a placeholder is never printed raw", "a token is upper-case letters, digits and underscores between double braces"). `CONTENT_TOKENS` is different in kind: there the braces ARE the content the merchant copies, so it stays as it is. Both were JSX string literals, so minification kept them; the bundle's inventory is now **18 tokens and every one is a documented row**, with no `{{TOKEN}}` or `{{UPPER_SNAKE}}` among them.
- `36 · 2026-09-08 · `src/index.js`'s crash screens were rebuilt rather than merely recoloured, and the interpolated message is now escaped · Task 5 asks for "neutral text, dark palette". The two handlers shared a copy-pasted `innerHTML` string on `#1a1a2e` with `color:red`/`color:orange` and headings that named the JS event ("JS Error (window.onerror)", "Unhandled Promise Rejection") — developer output on a screen a customer can reach. They are one `paintCrashScreen(detail)` now, mirroring `ErrorBoundary`'s card exactly (same palette literals, same `Fraunces`/`Manrope` stacks, same folded `<details>`), and the guard moved into the helper so both paths keep the "only into an empty #root" rule that stops this overwriting a rendered ErrorBoundary. Escaping was added in passing: the old code interpolated `msg`, `error.stack` and `String(event.reason)` straight into `innerHTML`.
- `36 · 2026-09-08 · The BRAND_FOOTPRINT §1 identifier list was re-checked name by name and SPLIT, rather than asserted clear wholesale · Sweeping all 29 as one line would have been wrong: 21 are gone from the tree outright (every token, the `.weave`/`.silk*`/`.loom` classes, `SILK_SPEC_LABELS`, `deriveFabricCraft`, `isPremiumProduct`, `GRADIENT_PRESETS`, `PROMISE_DETAIL`, `TRUST_BADGES`, `WHY_CHOOSE_US`, `MOCK_REVIEWERS` …), but eight are still there BY NAME and should be — `APP_TAGLINE`, `APP_DESCRIPTION`, `SUPPORT_EMAIL/PHONE/ADDRESS/HOURS`, `SOCIAL_LINKS`, `FAQ_ITEMS`. The footprint listed those because of the Meghali VALUES they carried; every one now reads straight off `src/config/brand.js`, so the value can only be the brand's or a documented token, and renaming them would buy nothing. Two further names survive only inside the comments that record their own deletion (`TRUST_BADGES`/`WHY_CHOOSE_US` in `constants.js:308-320`, `HERO_FALLBACK_SLIDES` in `heroConfig.js:27`), and `--sf-gradient-announce` is Prompt 03's single wash, not the Meghali `-1/2/3` triple. §1 now records the split.
- `36 · 2026-09-08 · db.json was not touched · Task 1 makes the edit conditional ("only if a hit remains") and no hit remained: the re-run of Prompt 06's validation passes every structural check and every Prompt 36 rule. `git status` shows the file unchanged, so both api modes and the admin are untouched by construction.
- `37 · 2026-09-08 · Tap targets are measured by HIT TESTING, not by geometry, and that decision found seven of the fifteen defects · The first harness computed a control's effective target from `getComputedStyle` on its `::before`/`::after` touch band. That is unsound: the values a browser returns for a positioned pseudo-element are PRE-TRANSFORM, so a band centred with `translateY(-50%)` reads as offset by half its height, and a band clipped by an ancestor reads as present. Rewritten to press the centre and the four edge midpoints of a 44px box with `document.elementFromPoint` (a pseudo-element hit answers with its originating element, and a `<label>` wrapping a native control counts as that control's target). Only then did the chapter-index pill's "44px overlay" show as 36, the breadcrumb's and the PDP story link's bands as clipped away, and the FAQ search row as a 52px control with a 24px hit area. A probe INSIDE a control's own box that answers with something else is recorded as `covered`, not as a size failure — the box is there, and paint order or a shared edge is a different question.`
- `37 · 2026-09-08 · The `svh` fallbacks on the three overlay primitives are `%` of their own fixed root, not `100vh`/`100vw` · The prompt asks for "`100vh` fallbacks". For `ui/Drawer`, `ui/Modal` and `pdp/Lightbox` the root is `position: fixed; inset: 0`, so `%` IS the viewport in every engine — and it is a BETTER fallback than `vh`, which on a phone is the LARGE viewport and would overflow the visible area, and better than `100vw`, which the same task forbids because it ignores a classic scrollbar. The hero and the full-page CTA, which are in normal flow, keep the literal `vh` → `svh` pair the prompt describes.`
- `37 · 2026-09-08 · The touch floor is put on the element's own BOX wherever an ancestor can clip, and only on an inert band where nothing can · The repo's existing idiom is an inert `::after` that grows the hit area without moving the layout. Measurement showed it fails in three places, all for the same reason — the band leaves the element and an ancestor clips it: a horizontal scroller (`overflow-x: auto` makes the Y axis `auto` too, so the chapter pill's band was cut back to the pill), a GlassCard's radius (`overflow: hidden`, which cut the breadcrumb's width band and the PDP story link's lower half) and `-webkit-line-clamp`, which REQUIRES `overflow: hidden` and so cannot host a band at all (the cart line name and the cross-sell name). Those five took a real `min-height`/`min-width`; the footer's small links, the gallery toggle and the admin switch kept the band, where nothing clips.`
- `37 · 2026-09-08 · 45 importance overrides were retired with class doubling, and the change was PROVED neutral rather than assumed · 28 of them were in `Header.module.css` fighting MUI's emotion classes. MUI emits one single-class rule per component, so a doubled class (`.avatar.avatar`) out-ranks it on specificity and stops caring which sheet the bundler emits last — the repo already uses that idiom on its `.sf-plate` overrides. Verified by snapshotting the account menu's computed styles (paper, avatar, divider, items, hover, user name and email — 19 properties each) before and after: byte-identical, with ONE difference that was then fixed — `overflow: hidden` had been silently losing its Y axis to MUI's `overflow-y: auto`, which is what lets a long menu scroll, so the rule now states the axis pair it was actually computing.`
- `37 · 2026-09-08 · Print is a TOKEN flip, not a per-module print stylesheet · `/order-confirmation/*` printed as warm-white ink on white paper — a blank receipt — because a printer does not paint backgrounds unless asked but always prints the text colour. Rather than write a print rule per module, the one place colour is declared (`theme/storefront-tokens.css`) re-points the whole `--sf-*` palette to ink-on-paper inside its own `@media print`, so every consumer prints correctly through the tokens it already reads and no component gains a hardcoded colour. The literals in that block are printer neutrals (white paper, three grey ink tiers, one deep gold at 7.9:1 — champagne gold is 1.5:1 on white), which is the same documented exception the admin invoice's stylesheet already takes. Two consequences had to be handled: `.sf-gradient-text` un-clips itself in print, or a gradient-filled headline prints as nothing; and `html body` re-states the ink, because MUI's CssBaseline emits a bare `body` rule after this sheet carrying the palette's literal warm white.`
- `37 · 2026-09-08 · The top safe-area inset goes on the announcement band AND on the header, but only while the header is pinned · Nothing on the storefront took `env(safe-area-inset-top)`. The band is the first thing in the document, so it is what a notch covers before anything scrolls; the header is what pins at y=0 for the rest of the scroll. Giving the header the inset unconditionally would have made it 59px taller while the band was still on screen, so `Header.js` now adds a `.pinned` class from the `scrolled` state it already tracks — `scrolled` alone, not `!overHero && scrolled`, because `overHero` decides how the header is PAINTED, not where it sits. The band's phone rule was also re-stating `padding` as a shorthand, which zeroed the inset again; both gutters are longhands now, with `max(gutter, inset)` on the sides for the landscape cut-outs. Every inset resolves to 0 until `public/index.html` opts into `viewport-fit=cover`, so none of this costs a pixel today.`
- `37 · 2026-09-08 · Body-copy fixes were limited to OFF-SCALE sizes; the design system's own 14px support tier stays · `Pillars .text` and `EmptyState .text` were hand-rolled `0.9375rem` — neither the 16px reading tier nor the 14px support tier — and both are running body copy, so both take `--sf-text-base` at ≤414. Everything else the audit reported under 16px is `--sf-text-sm` at weight 400 or 500: footer small print, form hints, card blurbs, notes and captions, which is what that tier is for (`DESIGN_SYSTEM.md` §6). The rule the prompt actually names — no LIGHT text under 14px — cannot be broken at all: `--sf-font-light` is deleted, and the audit found 0 elements below weight 400 at any size at any width.`
- `37 · 2026-09-08 · The eight remaining comment mentions of the importance keyword were reworded so the acceptance grep counts DECLARATIONS · After the sweep, `grep -rn "!important" src --include=*.css` still returned 8 lines, every one of them prose explaining why the override is NOT used ("Doubled so it beats X without an …"). A maintainer running that grep wants declarations, so eight false positives make the search worse, not better; each comment now names the mechanism ("without raising importance") and says exactly what it said before. The count is 0 declarations and 0 lines. Recorded here because it is a deliberate edit to comments, not a side effect.`
- `37 · 2026-09-08 · The admin's touch floor is keyed to the POINTER, not to the viewport width · It was `@media (max-width: 768px)` at 40px, which misses a 1024px tablet and catches a narrow desktop window. Now `@media (pointer: coarse)` at 44px, extended from icon buttons and small buttons to `Button`, `ListItemButton`, `InputBase`, `Chip` (clickable and deletable only — a status chip is a label, not a target), `ToggleButton` and `FormControlLabel`. The field's floor goes on the InputBase ROOT and MUI's own `align-items: center` keeps the input centred in it — stretching the input instead was tried and is worse, because MUI gives it an explicit height, so it stayed 40px and merely moved to the top of the box. The bare switch on Shipping grows its own hidden input to a centred 44px band, so nothing painted moves and the thumb keeps its alignment with the track.`
- `38 · 2026-09-08 · Every number in AUDIT.md is a MEASUREMENT, and the harnesses are not committed · Four scripts drove the production build in Chromium: axe over 47 states (every listed route plus every overlay, every checkout step, the lightbox and all 20 admin screens, signing in where the state needs it), a keyboard harness that uses nothing but Tab/Shift+Tab/Enter/Escape/arrows, a reduced-motion harness that loads nine surfaces twice and reads `document.getAnimations()` plus video and canvas state, and an SEO harness that reads the live `<head>` on 22 routes and parses and shape-checks every JSON-LD graph. They stay OUT of the repository because they need `playwright`, `axe-core` and `lighthouse`, and the programme adds no dependency for an audit (`00_INDEX.md` §2 — CLI tools via `npx` only); `AUDIT.md` §2 records what each one did and how to re-run the same checks through `npx`.`
- `38 · 2026-09-08 · TYPE SIZE IS NOT DOCUMENT STRUCTURE — one theme default replaced a heading-order bug on nine admin screens · MUI maps `variant="h4"` to an `<h4>` element and `subtitle2` to an `<h6>`, so every admin stat figure ("₹1,309", "8", "2"), every accordion label and every product name in a hero row shipped as a document heading four or five levels under a page that had no `<h1>` at all. Chasing the twelve nodes one at a time would have left the next one free to appear, so `adminTheme.js` now sets `MuiTypography.defaultProps.variantMapping` — only h1/h2/h3 are headings; everything else keeps its size and becomes a `<p>`. A heading is then something a screen states ON PURPOSE: `component="h1"` on all 20 page titles and `component="h2"` on the 13 real panel headings. Nothing on screen moved — `variant` still picks the type, `component` only picks the tag.`
- `38 · 2026-09-08 · The two colour-contrast failures were fixed with the SCRIM, on the shared shells, not with colour · The cart tray's 12px muted note composited to 3.82:1 over a gold-lit rgb(91, 83, 57) — the PDP's pack shot reading through the glass — and the auth modal's field labels failed the same way over `/checkout`. No colour on the text side could fix either: the background IS the photograph, and `--sf-color-text-secondary` measures ~3.8:1 against it too. `sf-glass--scrim` (the design system's own `--sf-color-bg` at 35%, DESIGN_SYSTEM §4) takes the same pair to 5.7:1, and it went on `ui/Drawer.js` and `ui/Modal.js` rather than on the two stylesheets that happened to fail, because the defect is the class "glass over live content", not those two lines.`
- `38 · 2026-09-08 · CLS 0.229 → 0.013 on /shop by reserving each slot from the component that OWNS it, and tying each reservation to the request it waits on · A `PerformanceObserver` trace named three inserts: the 36px announcement band (on every route, 0.041), the 200px concern chip row and the 48px chapter strip. The band now paints its first frame from `brand.announcements` — the module the bundle already has, and already the component's fallback when the API is empty. The strip reserves its constant height while the range loads, and ONLY while loading: a failed fetch and an empty range both end with no band, and reserving one that will never come is its own gap. The chip row was the instructive one — reserving it against the products' `status` made the head SHRINK when the products landed and grow again when the concerns did, two shifts where there had been one, because the concerns are a second independent fetch; `useShopData` now returns its own `concernsLoading` and the reservation waits on that. Measured: reserved row 200px, real row 200px.`
- `38 · 2026-09-08 · The Google Fonts stylesheet stopped blocking the first paint, and that was the single largest performance change · Lighthouse put 2.19 s of render-blocking CSS on the critical path, ~950 ms of it this one file — from a third origin, after a DNS + TLS handshake, before a single LAMIKAA pixel. `display=swap` already described the intended behaviour (paint in the token fallback, re-render when the faces land); `media="print"` + `onload="this.media='all'"`, with a `preload` beside it and a `<noscript>` copy, is how the STYLESHEET stops blocking too. Still one request — the browser serves the preload and the stylesheet from one fetch. FCP fell from 2.8–3.1 s to 1.4–2.1 s across all four audited pages.`
- `38 · 2026-09-08 · `sweetalert2` moved behind one memoised `import()`; the CartContext test now mocks the SEAM, not the library · Five providers that mount on every route imported it eagerly — 79 kB parsed, 42% of `main.js` unused on first load, for a library whose every use is a reaction to a click. `src/utils/alerts.js` exposes `fireAlert(options)` returning SweetAlert's own result promise, so `const { isConfirmed } = await fireAlert({…})` reads exactly as before; the load cannot fail loudly (a chunk that does not arrive logs and resolves as a dismissed dialog, never an unhandled rejection in a click handler), and a failed import is not cached so a later call can still succeed. `main.js` 247.8 → 233.1 kB gzipped. `CartContext.test.js` mocks `utils/alerts` instead of `sweetalert2`, which is both the seam the context now uses and 79 kB a unit test no longer pulls in.`
- `38 · 2026-09-08 · Taking the fonts off the critical path CREATED a layout shift, and the fix was metric-matched fallback faces rather than a retreat · `display=swap` was already the policy, but while the stylesheet was render-blocking the swap happened BEFORE the first paint, so nothing ever reflowed. Non-blocking moved it after, and text that is laid out in a fallback and re-laid-out in Manrope reflows: `/shop`'s row of twelve concern pills re-wrapped from three lines to four, 0.16 CLS on its own. It was not a localhost artefact — delaying every API response by 120 ms and 300 ms changed the number by 0.0001, which ruled that out. Reserving the row cannot help, because the row IS the thing that changes size. `storefront-tokens.css` now declares two `@font-face` rules with no `src` file: `local()` anchors (Arial/Helvetica/Liberation Sans; Times/Liberation Serif — the two families whose metrics are stable across platforms by design) and `size-adjust` ratios MEASURED in Chromium against the loaded webfaces, 101.1% and 113.3%. Cost: zero requests, zero bytes. Verifying it exposed a second bug in the reservation itself — it was written in `ch`, which is the advance of a ZERO and therefore changes with the font FAMILY, so the reservation was still in fallback metrics when the row it reserved had already switched. In `em` it is family-independent, and reserve-against-real now measures 200/200 px at 412, 96/96 at 768 and 44/44 at 1280. A third came out of the same measurement: the empty reserve pill was `inline-flex`, so every wrapped row carried a text baseline's leading (+6 px x 4 rows).`

- `38 · 2026-09-08 · Performance did not reach 85, and the gap was measured rather than guessed · After the four wins the medians over three runs are 73 (`/`), 61 (`/shop`), 56 (PDP) and 75 (`/checkout`), from 53/42/52/63. Lighthouse's own LCP breakdown attributes almost the whole of it to `resourceLoadDelay` — 910 ms observed against 78 ms of `resourceLoadDuration` and 75 ms of `elementRenderDelay`: the hero image is fast, and the wait before anyone can ASK for it is the SPA booting and the catalogue read returning, because the hero is product-driven by design (`heroOrder`). TBT is the eager bundle's evaluation under a 4× CPU throttle, and its three largest passengers are react-dom (127 kB parsed), framer-motion (107 kB) and MUI (105 kB). None of those is a defect this audit introduced or can responsibly reverse in passing, so §7.4 costs the three levers instead — prerender or SSR so `index.html` can preload the hero, take MUI off the storefront's eager path (≈33 kB gzipped), bundle the statically named Iconify icons — and §11 hands them to the owner. Publishing a passing number was never available; recording the real one with its cause is.`
- `38 · 2026-09-08 · `/checkout` scores 69 on Lighthouse SEO and that is the CORRECT score · The only failing audit is `is-crawlable`, and it fails because the page carries `noindex,nofollow` — which this prompt requires (task 4) and which is right for a step in one visitor's transaction. Lighthouse cannot tell a deliberate `noindex` from an accidental one. Every other SEO audit on the page passes and `/checkout` is `Disallow`-ed in robots.txt as well. Removing the directive to make a number go up would be the wrong trade and is not done.`
- `38 · 2026-09-08 · `/login` and `/register` take a `Disallow`, not a `noindex` · They are not pages: `AuthRoute` opens the modal and immediately `<Navigate replace>`s back, so there is no document to carry a robots tag. The crawl directive is still worth having — it saves a crawler the fetch that discovers there is nothing there — so both, plus `/wishlist` and `/search`, were added to `robots.txt` this prompt. `/search` matters twice over: every `?q=` permutation is a duplicate of the listing it filters.`
- `38 · 2026-09-08 · Four findings were reviewed and deliberately NOT changed, and each is argued in AUDIT.md §9 · axe's experimental `p-as-heading` fires on the reviews empty state and on product-card name rows — a card name is not a document heading, and making every one into an h3 would flood the outline with a dozen peers of the page's own sections. `focus-order-semantics` fires on the gallery stage, which is a focusable `role="group"` with `aria-roledescription="carousel"` and a key handler: the WAI-ARIA APG carousel pattern exactly. `bf-cache` is a property of the `serve` dev server used for the audit. `cache-insight` is a `Cache-Control` policy for the owner's web server, which no file in this repository can set — it is TODO 3 in §11. Recording a rejected finding with its reason is what stops the next audit re-opening it.`

## Open TODOs

Carry-overs that a later prompt (or the developer/owner) must pick up (format: `NN · item · owner · target prompt`).

- `36 · `public/robots.txt` has NO `Sitemap:` line and `public/index.html` has NO `og:url`/`twitter:url`: Prompt 36 removed all three because they carried a literal `{{LAMIKAA_DOMAIN}}`. When the owner names the domain, set `brand.seo.siteUrl` in `src/config/brand.js` (one line — every canonical, `og:url` and JSON-LD `url` follows it automatically), then let Prompt 38's `scripts/generate-sitemap.js` write `public/sitemap.xml` and re-add the `Sitemap:` line with the real host. Re-adding a static `og:url`/`twitter:url` pair is optional — `useSeo` already writes a per-route `og:url`, which is better than one site-wide value. · Prompt 38 / owner · 38` — **Prompt 38 did its half and the rest is now one owner decision.** `scripts/generate-sitemap.js` exists and was verified in both directions; `robots.txt` carries the `Sitemap:` line **written out and commented, with the candidate host in it**, so nothing has to be composed later. The static `og:url`/`twitter:url` pair was left off, deliberately: `useSeo` writes a per-route `og:url` on every page, and a site-wide one in `index.html` would be a worse value that a crawler might prefer. What is left for the owner is the three ordered steps in `AUDIT.md` §10 and TODO 1 in §11.

- `38 · **The public domain.** `brand.seo.siteUrl` is still `{{LAMIKAA_DOMAIN}}`. Resolving it is three ordered steps and no page changes: set it in `src/config/brand.js`, run `npm run sitemap` (writes `public/sitemap.xml`), then uncomment the `Sitemap:` line already written into `public/robots.txt` with the same host. Candidate `lamikanaturals.com` — `.env.production` points the API at `core.lamikanaturals.com`. Until then the script writes nothing and prints a notice, and `seoOrigin()` falls back to `window.location.origin`, so no canonical, `og:url` or JSON-LD `url` has ever carried the token. · owner · 39`

- `38 · **Cache headers on the hosting.** Lighthouse's `cache-insight` fails because `serve` sets no far-future `Cache-Control` on the hashed build output. Nothing in this repository can set it: the owner's web server should send `public, max-age=31536000, immutable` for `build/static/**` and `no-cache` for `index.html`. Costs nothing on a first visit and is most of the repeat-visit score. · owner/hosting · —`

- `38 · **Performance beyond an audit's remit — three costed levers, in order of effect.** (1) Prerender or server-render the shell so `index.html` can carry `<link rel=preload as=image>` for the first hero slide; Lighthouse attributes 910 ms of observed LCP to the wait before the URL is even known. (2) Take `@mui/material` off the storefront's EAGER path — it is there for `useMediaQuery` in the header and footer, five components in the account dropdown and `createTheme`/`ThemeProvider` in `ThemeContext`, and everything else on the storefront is CSS Modules and this repo's own primitives; ≈33 kB gzipped. (3) Bundle the ~114 statically named Iconify icons with `addCollection()`, keeping the API as the fallback — another ~118 call sites take their icon name from a prop and some of those names are admin-managed data (`category.icon`, siteContent section icons, deal tiles), so the API cannot simply be removed. Each is a change with its own design decisions; `AUDIT.md` §7.4 has the measurements. · developer/owner · 39`

- `38 · **`@iconify/react` fetches icon data from `api.iconify.design` at runtime** — three requests on the home page, the first 544 ms under Lighthouse's throttling, from an origin otherwise not on the critical path. Beyond the bundle question above, it is also a hard third-party runtime dependency: with that host unreachable, every icon-only control collapses to its padding (Prompt 37 hit exactly this in the sandbox). Worth a decision before launch even if the bundling is not done. · developer/owner · 39`
...
- ~~`20 · No placeholder photograph could be seen rendered in the browser in this sandbox …` · developer · 37~~ — **RESOLVED by Prompt 37.** The QA harness fetches every `res.cloudinary.com`, `picsum.photos` and `api.iconify.design` URL with `curl` (which has the proxy's CA), caches it on disk and fulfils the browser's request from that cache. Every product photograph, every placeholder photograph and every icon rendered for real across 364 route × width states, so the aspect-ratio check (`naturalWidth/naturalHeight` against the rendered box for any distorting `object-fit`) and the icon-sized-control measurements are measurements rather than assumptions: **0 broken, 0 stretched**. Icons matter twice over — with Iconify unreachable, every icon-only control collapses to its padding, which made the whole admin read as failing the tap-target check until the harness was fixed.
...
- `36 · {{LAMIKAA_DOMAIN}} · location narrowed · The token's ONLY home is now `brand.js → seo.siteUrl`. The `og:url` and `twitter:url` tags in `public/index.html` and the `Sitemap:` line in `public/robots.txt` carried it literally; both files are served verbatim to crawlers, so a raw token there is a broken directive rather than a harmless stand-in, and all three were removed. Nothing rendered changes: `useSeo`'s `seoOrigin()` has written `og:url` and the canonical from `brand.seo.siteUrl` (falling back to `window.location.origin`) since Prompt 08, and Prompt 38 re-adds the `Sitemap:` line with the real host when it writes `public/sitemap.xml`. Resolving the domain stays a one-line edit in `brand.js`.`
- `36 · every token · verified · The production bundle carries **18** distinct `{{TOKEN}}` strings and each one is a row of `_reference/PLACEHOLDERS.md` (`grep -rhoE "\{\{[A-Z0-9_]+\}\}" build/static/js | sort -u`). `{{TOKEN}}` and `{{UPPER_SNAKE}}` used to be in that list as literal examples in admin prose and are now reworded out. `db.json` carries 16, all documented. None of them renders on any storefront route.`
...
## Prompt 37 record (2026-09-08)

### What the pass actually did

Drove every storefront route, every overlay and all nineteen admin screens through Chromium 141 at the
seven reference widths, plus phone landscape, an emulated iPhone 14 Pro notch (portrait and landscape),
reduced motion, print and a long-content stress pass — **and fixed the fifteen defects the measurements
found**. Nothing in `QA_MATRIX.md` is asserted from reading the CSS.

| Pass | States | What was measured | Result |
|---|---|---|---|
| Route × width | **364** (33 storefront rows + 19 admin × 7 widths) | horizontal scroll (twice — with the `body` guard and without it), clipped text, image aspect and loading, type sizes, console errors | **0 / 0 / 0 / 0 / 0** |
| Overlays | **39** | dialog present, scroll locked (wheel AND touch), focus inside, Escape closes, lock released, focus restored, no overflow — eight checks each | all pass |
| Tap targets | **160** (21 storefront routes + 19 admin screens × four coarse widths) | **hit-tested**: centre + four edge midpoints of a 44px box, `document.elementFromPoint` | **10 603 controls, 0 short, 0 conflicts** |
| Keyboard focus | 17 routes | before/after signature over the control, its subtree and four ancestors | **1 060 focusables, 936 stops, 0 without an indicator** |
| Long content | **49** | a 128-char product name and a 100-char unbroken token substituted on the API and in the cart and wishlist | **0 overflow, 0 clipped** |
| Landscape 844×390 | 5 routes | overflow, page scrolls to its end, search overlay and lightbox fit | all pass |
| Safe area | 2 orientations × 3 routes, at rest and scrolled | `Emulation.setSafeAreaInsetsOverride` (59/34 portrait, 59/59/21 landscape) | band 59, pinned header 59, tab bar 34/21, PDP and cart bars 42/29, main 114 |
| Print | confirmation + admin invoice | ink luminance over every text run, ground luminance over every large box, chrome visibility | white paper, `rgb(17,17,17)` ink, 0 pale runs, 0 dark grounds, 0 chrome |
| Sticky overlap | 3 routes × 4 widths | every field and button focused, `elementFromPoint` at its centre | 0 covered |

### The three measurement mistakes that mattered

Worth recording, because each one hid real defects and each was only caught by disbelieving a clean result:

1. **Computing a touch band from `getComputedStyle` is unsound.** The values are pre-transform, so the
   footer's `top: 50%; transform: translateY(-50%)` band read as offset by 22px and a clipped band read
   as present. Rewritten as a hit test — which is also what the criterion actually means.
2. **Remote media has to render.** With Cloudinary, picsum and Iconify unreachable, every icon-only
   control collapses to its padding and every product plate reports `naturalWidth: 0`. The whole admin
   read as failing the tap-target check for that reason alone. Fixed by fetching through `curl` and
   fulfilling from a disk cache — which also closed the Prompt 20 carry-over.
3. **`overflow: hidden` does not stop `window.scrollTo`.** It removes the USER's scrolling mechanism,
   which is the wheel and the finger. The first overlay harness reported the scroll lock as broken on
   every drawer; driving a wheel and a synthetic touch drag instead shows all 39 states locking and
   restoring correctly.

### Files changed (34)

**Storefront CSS (24):** `App.css` · `theme/storefront-tokens.css` (the print palette) ·
`theme/storefront-primitives.css` (`.sf-gradient-text` in print) · `AnnouncementBar` · `Header` ·
`MegaPanel` · `TrustStrip` · `Footer` · `BottomNav` · `Breadcrumb` · `Logo` · `Pillars` ·
`cart/CrossSell` · `catalogue/CategoryHead` · `catalogue/ChapterIndex` · `home/HeroCarousel` ·
`pdp/Lightbox` · `pdp/MediaGallery` · `pdp/PurchasePanel` · `storefront/AddToCartBar` · `ui/Drawer` ·
`ui/EmptyState` · `ui/Modal` · plus `pages/{About,Cart,Checkout,Faq,OrderHistory,Policies,Profile,Rituals,WhyLamikaa}`.
**JS (2):** `components/Header/Header.js` (the `.pinned` class) and `theme/adminTheme.js` (the
pointer-keyed 44px floors).
**Docs (2):** `prompts/_reference/QA_MATRIX.md` (new) and this file.

No component was removed, no feature was dropped and no page's information architecture changed; every
edit is a target size, a viewport-unit fallback, a device inset, a print value or a specificity rewrite.

### Verification

| Check | Result |
|---|---|
| `CI=true npm run build` | exit 0, **no warnings** (one `no-dupe-keys` error was introduced and fixed during the pass — see defect 15) |
| `npm test -- --watchAll=false` | exit 0 — 27 suites / **321** tests passed, 1 suite / 50 skipped |
| `grep -rn "100vw" src --include=*.css \| wc -l` | **3** — every one prose in a comment explaining why `100%` is used instead; **0 declarations** |
| `grep -rn "!important" src --include=*.css \| wc -l` | **0** (was 45 declarations plus 8 prose mentions) |
| `git status db.json` | clean — JSON Server ran against a scratch copy through `JSON_SERVER_DB` for the whole pass |

### For Prompt 38

- The harness (`scratchpad/qa/{run,overlays,taps,focus,stress,extras,print,admin-taps}.js`) is reusable
  as it stands: the media-cache route handler is what makes any Chromium run in this sandbox honest, and
  the tap and focus tests are the two the accessibility audit will want to re-run after its own changes.
- Two things it recorded but did not change, both in `QA_MATRIX.md` §15: `api.js` logs an
  already-handled cart 404 through `console.error` before re-throwing, and the wishlist keys its rows by
  `productId` while the API permits two rows per product per user. Neither is a layout defect.
