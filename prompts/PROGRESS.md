...
- `36 · `public/robots.txt` has NO `Sitemap:` line and `public/index.html` has NO `og:url`/`twitter:url`: Prompt 36 removed all three because they carried a literal `{{LAMIKAA_DOMAIN}}`. When the owner names the domain, set `brand.seo.siteUrl` in `src/config/brand.js` (one line — every canonical, `og:url` and JSON-LD `url` follows it automatically), then let Prompt 38's `scripts/generate-sitemap.js` write `public/sitemap.xml` and re-add the `Sitemap:` line with the real host. Re-adding a static `og:url`/`twitter:url` pair is optional — `useSeo` already writes a per-route `og:url`, which is better than one site-wide value. · Prompt 38 / owner · 38`
